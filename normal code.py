...
א. קטגוריה: בחרו את נושא המילים שתרצו, למשל אוכל, חפצים, חיות ועוד.
ב. רמת קושי: בחרו את רמת הקושי המתאימה לכם.
ג. שם שחקן: הקלידו את השם שלכם.
ד. מצב אימון: אפשר לסמן מצב אימון כדי לשחק בכיף ללא חישוב ניקוד.
ה. רמזים: בחרו רמזים מוכנים מראש או AI שמשפר את הרמזים.
ו. אורך משחק: בחרו אם לשחק את כל הקטגוריה או מספר מילים מוגבל.
...
שלב 2: Multiplayer דרך שרת

כדי לשחק Multiplayer צריך לסמן Multiplayer דרך שרת.
בשדה כתובת שרת מכניסים את הכתובת של השרת, למשל:
http://34.165.123.53:8000

אחר כך לוחצים בדוק שרת.
אם הבדיקה הצליחה, אפשר להתחיל משחק.

קוד חדר:
אם משאירים את קוד החדר ריק, השרת ייצור חדר חדש בתחילת המשחק.
אם רוצים להצטרף לחדר קיים, כותבים את קוד החדר שקיבלתם מהשחקן השני.

ב-Multiplayer המילה הסודית נשארת רק בשרת.
השחקנים רואים רק את הרמזים, הניחושים והניקוד.
אם בוחרים רמזי AI, השרת ינסה להשתמש ב-AI; אם זה לא מצליח, הוא משתמש ברמזים המוכנים.
...
שלב 4: שיטת הניקוד והזמן

הניקוד נקבע לפי כמות הרמזים שנאלצתם לראות כדי לנחש את המילה:
א. ניחוש נכון לפי רמז ראשון: 10 נקודות.
ב. ניחוש נכון לפי רמז שני: 7 נקודות.
ג. ניחוש נכון לפי רמז שלישי: 4 נקודות.
ד. ניחוש נכון לפי רמז רביעי: 2 נקודות.
ה. ניחוש נכון לפי רמז חמישי: 0 נקודות.
